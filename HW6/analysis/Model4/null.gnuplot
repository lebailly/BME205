...
set title "protein encoding codon model"
num_prot = 2994

unset logscale x
set logscale y
m=50; beta=30;
# fit  gumbel(x,m,beta) 'Model4.txt' using 1:3 via m,beta
# fit  gumbel_P(x,m,beta) 'Model4.txt' using 1:4 via m,beta
fit  log(gumbel_P(x,m,beta)) 'Model4.txt' using 1:(log($4)) via m,beta
plot [10:400] gumbel(x,m,beta), 'Model4.txt' using 1:3
...
set ylabel "E-value"
plot [10:400] gumbel_P(x,m,beta), 'Model4.txt' using 1:4
...
# fit lognormal(x,mu,sigma) 'Model4.txt' using 1:3 via mu,sigma
# fit lognormal_P(x,mu,sigma) 'Model4.txt' using 1:4 via mu,sigma
fit log(lognormal_P(x,mu,sigma)) 'Model4.txt' using 1:(log($4)) via mu,sigma

set logscale xy
set ylabel "probability"
plot [10:400] lognormal(x,mu,sigma), 'Model4.txt' using 1:3
...
set ylabel "E-value"
plot [10:400] lognormal_P(x,mu,sigma), 'Model4.txt' using 1:4
